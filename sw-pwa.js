...
// Estrategia: cache-first para assets estáticos, network-first
// para HTML, con fallback offline.
// ============================================================

const CACHE_NAME = 'jrubio-pwa-v1';
const STATIC_CACHE = 'jrubio-static-v1';
const RUNTIME_CACHE = 'jrubio-runtime-v1';

// Assets críticos que se cachean en install (offline-ready)
const PRECACHE_URLS = [
  './',
  './index.html',
  './manifest.json',
...
async function handleFetch(request) {
  ...
  if (isHTML) {
    try {
      ...
      return new Response('Sin conexión. Recargá cuando tengas internet.', {
        status: 503,
  ...
  // 3. Si no está en cache, ir a network
  try {
    const fresh = await fetch(request);
    if (fresh.ok) {
